...
// Prints the value of each of property of the Player object.
...
func (tp *Player) OfferDoubleDown(d blackjack.Dealer, h blackjack.Hand) bool {
	...
	dealerFaceUp := d.FaceUp().Evaluate()

	// IF the dealer is showing an Ace, forget about it
	if d.FaceUp().Evaluate() == 1 {
		return false
	}
...
func (tp *Player) Done(doneMsg string) {
	fmt.Printf("Game is over for ai with id %v -- points earned: %v -- message from game: %v\n", tp.id, tp.points, doneMsg)
}
